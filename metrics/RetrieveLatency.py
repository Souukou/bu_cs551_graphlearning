...
def monitor_latency(flink_jobmanager_url, job_id, vertex_id, output_file, polling_interval, metric):
    def fetch_latency(jobmanager_url, job_id, vertex_id, metric):
        url = f'{jobmanager_url}/jobs/{job_id}/vertices/{vertex_id}/metrics?get={metric}'
        response = requests.get(url)
        if response.status_code == 200:
            return response.json()
        else:
            print(f'Error fetching latency data: {response.status_code}')
            return None

    with open(output_file, 'w', newline='') as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(['Timestamp', 'latency'])

        while True:
            latency_data = fetch_latency(flink_jobmanager_url, job_id, vertex_id, metric)
            if latency_data:
                timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
                csv_writer.writerow([timestamp, latency_data])
                f.flush()
        ...
        if vertex['name'] in operators:
            ...
            thread_dict[thread_name] = threading.Thread(
                ...
                args=(
                    ...
                    job_id,
                    vertex['id'],
                    './results/' + vertex['name'] + '_latency.csv',
                    polling_interval,
                    metric,
                ),
            )
